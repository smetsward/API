...
@app.put("/items/{id}", response_model=schemas.ItemBase)
async def update_item(id: int, item: schemas.ItemBase, db: Session = Depends(get_db)):
    return crud.update_item(db, id=id, item=item)


@app.delete("/items/{id}")
async def delete_item(id: int, db: Session = Depends(get_db)):
    return crud.delete_item(db, id=id)
